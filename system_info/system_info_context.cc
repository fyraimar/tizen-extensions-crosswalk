...
void SystemInfoContext::GetDisplay(picojson::value& error,
  ...
  ret = (double)disp.GetResolutionWidth(error);
  if (!error.get("message").to_str().empty())
    return;
  data_map["resolutionWidth"] = picojson::value(ret);

  ret = (double)disp.GetResolutionHeight(error);
  if (!error.get("message").to_str().empty())
    return;
  data_map["resolutionHeight"] = picojson::value(ret);

  ret = (double)disp.GetDotsPerInchWidth(error);
  if (!error.get("message").to_str().empty())
    return;
  ...
  ret = (double)disp.GetDotsPerInchHeight(error);
  if (!error.get("message").to_str().empty())
    return;
  data_map["dotsPerInchHeight"] = picojson::value(ret);

  ret = (double)disp.GetPhysicalWidth(error);
  if (!error.get("message").to_str().empty())
    return;
  data_map["physicalWidth"] = picojson::value(ret);

  ret = (double)disp.GetPhysicalHeight(error);
  if (!error.get("message").to_str().empty())
    return;
  data_map["physicalHeight"] = picojson::value(ret);

  ret = (double)disp.GetBrightness(error);
  if (!error.get("message").to_str().empty())
    return;
